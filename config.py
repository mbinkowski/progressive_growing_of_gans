...
data_dir = '/nfs/data/mikolajb/progressive_growing/data'
result_dir = '/nfs/data/mikolajb/progressive_growing/results'
...
if mmd:
    desc    = 'pwmmdgan'                                        # Description string included in result subdir name.
    D       = EasyDict(func='networks.D_paper', fmap_out=16, return_gradient=True) # Options for discriminator network.
    G_loss  = EasyDict(func='loss.G_mmdgan', kernel_type='rbf', hessian_scale=True) # Options for generator loss.
    D_loss  = EasyDict(func='loss.D_mmdgan', kernel_type='rbf', hessian_scale=True) # Options for discriminator loss.
else:
    desc    = 'pgan'
